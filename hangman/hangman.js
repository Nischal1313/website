...
const hangmanLetters = ["H", "A", "N", "G", "M", "A", "N"]
const theWord = getRandomWord()
let guessedTimes = 0
let x = 0

function getRandomWord() {
    let word = Math.floor(Math.random() * words.data.length);
    return words.data[word];
}

function getCharacters() {
    for (let i = 0; i < theWord.length; i++) {
        const character = theWord.charAt(i);
        correctWord.push(character.toUpperCase())
    }
...
function clearTextField() {
    let clearTextField = document.getElementById('word');
    clearTextField.value = "";
}

function createHangmanBoxes(numberOfBoxes) {
    for (let i = 0; i < numberOfBoxes; i++) {
        let div = document.createElement("div")
        div.className = `hangman-box`
        div.id = `hangman-box${i}`
        div.innerText = '?'
        div.style.textDecoration = 'underline'
        div.style.textDecorationColor = 'rgb(0,250,0)'
        let insert = document.getElementById('hangman-boxes')
        insert.append(div)
    }
}

function guessedCharacter(letter) {
    guessedCharacters.push(letter)
    const display = document.getElementById('guessedCharacters')
    display.innerText += `${letter},\u00A0`
}

function wonOrNot(rightWord, guessedWord) {
    if (rightWord.length !== guessedWord.length) {
        return false;
    }
    for (let i = 0; i < rightWord.length; i++) {
        if (rightWord[i] !== guessedWord[i]) {
            return false;
        }
    }
    return true
...
function displayWin() {
    if (wonOrNot(guessedCharacters, correctWord)) {
        const display = document.getElementById('gamestate')
        display.innerText = "You got the word. Congratulations!"
    }
}
...
function hardMode() {
    for (let i = 0; i < 7; i++) {
        ...
        div.id = `hard-box${i}`
        div.innerText = '?'
        let insert = document.getElementById('hard-boxes')
        insert.append(div)
    }
}

function deleteHardboxes() {
    for (let i = 0; i < 7; i++) {
        let div = document.getElementById(`hard-box${i}`);
        if (div) {
            div.remove();
        }
    }
...
function handleChange() {
    ...
    if (checkbox.checked && guessedTimes === 0) {
        hardMode();
    }
    if (!checkbox.checked && guessedTimes < 1) {
        deleteHardboxes()
    }
    if (checkbox.checked && guessedTimes > 0) {
        checkbox.checked = false;
        const display = document.getElementById('gameStarted')
        display.innerText = 'You cannot do this when the game has started.'
...
function revealWord() {
    for (let i = 0; i < correctWord.length; i++) {
        let div = document.getElementById(`hangman-box${i}`);
        div.innerText = `${correctWord[i]}`;
        div.style.textDecoration = 'none'
    }
}

function processGuess(correctList, guessedLetter) {
    let reduceLives = true;
    for (let i = 0; i < correctList.length; i++) {
        if (guessedLetter === correctList[i]) {
            let div = document.getElementById(`hangman-box${i}`);
            div.innerText = `${guessedLetter}`;
            div.style.textDecoration = 'none'
            reduceLives = false;
        }
    }
    const checkbox = document.getElementById('toogle');
    if (reduceLives && checkbox.checked && guessedTimes < 8) {
        let display = document.getElementById(`hard-box${x}`);
        display.innerText = `${hangmanLetters[x]}`;
        display.style.color = 'rgb(255,0,0)'
        x = x + 1;
    }
}

function winningbox(word, guesses) {
    const uppercasedWord = word.map(char => char.toUpperCase());
    const uppercasedGuesses = guesses.map(char => char.toUpperCase());
    ...
    if (uppercasedWord.every(char => uppercasedGuesses.includes(char))) {
        modalMessage.innerText = 'Congratulations! You got the word.';
    ...
    } else if (guessedTimes > 7) {
        revealWord()
        modal.style.display = 'block';
        modalMessage.innerText = 'Hangman occurred! You did not get the word.';
        modalMessage.style.color = 'red'
        modalContent.style.borderColor = 'rgb(255,0,0)'
...
const roundWord = getCharacters()
createHangmanBoxes(roundWord)
let submit = document.getElementById('guess')
submit.addEventListener('click', function (event) {
    guessedTimes += 1
    event.preventDefault()
    const guessedLetter = document.getElementById('word').value.toUpperCase()
    guessedCharacter(guessedLetter)
    displayWin()
    clearTextField()
    processGuess(correctWord, guessedLetter)
    winningbox(correctWord, guessedCharacters)
})
...
document.getElementById('word').addEventListener('blur', function () {
    if (!this.value) {
        this.setAttribute('placeholder', 'Enter your character here:');
    }
});